...
output "tokyo_tgw_id" {
  ...
  value       = module.shinjuku_tgw.id
  sensitive   = false
}

output "tokyo_tgw_arn" {
  description = "Tokyo Transit Gateway ARN"
  value       = module.shinjuku_tgw.arn
  sensitive   = false
}
...
output "rds_security_group_id" {
  description = "Tokyo RDS security group ID for cross-region access"
  value       = module.security.rds_security_group_id
  sensitive   = false
}

output "rds_endpoint_full" {
  description = "Tokyo RDS endpoint (host:port)"
  value       = "${module.rds.address}:3306"
  sensitive   = false
}