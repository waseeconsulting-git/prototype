...
module "vpc" {
  ...
  shinjuku_origin_header_value01_result = module.cloudront.shinjuku_origin_header_value01_result

  id = module.shinjuku_tgw.id
}
...
module "dns" {
  ...
  #LAB2
  shinjuku_cf01_domain_name = module.cloudront.shinjuku_cf01_domain_name
  shinjuku_cf01_hosted_zone_id = module.cloudront.shinjuku_cf01_hosted_zone_id
}
...
module "cloudront" {
  ...
  env_prefix = var.env_prefix
  cloudfront_acm_cert_arn = module.dns.cloudfront_acm_cert_arn
  account_id = var.account_id
  alb_arn = module.vpc.alb_arn
  alb_dns_name = module.vpc.alb_dns_name

  # NEW: Lab 2B Cache Policies
  cache_policy_static_id          = aws_cloudfront_cache_policy.shinjuku_cache_static01.id
  cache_policy_api_id             = aws_cloudfront_cache_policy.shinjuku_cache_api_disabled01.id
  origin_request_policy_static_id = aws_cloudfront_origin_request_policy.shinjuku_orp_static01.id
  origin_request_policy_api_id    = aws_cloudfront_origin_request_policy.shinjuku_orp_api01.id
  response_headers_policy_static_id = aws_cloudfront_response_headers_policy.shinjuku_rsp_static01.id
  
  # NEW: Define ordered cache behaviors
...
# ==================== Lab 3A: Transit Gateway Components ====================
...
module "shinjuku_tgw" {
  ...
  name        = "shinjuku-tgw01"
  description = "Tokyo Transit Gateway hub (Shinjuku Station)"
  region      = var.region
  
  amazon_side_asn = 64512  # Custom ASN for Tokyo
  
  tags = merge(var.tags, {
    Component = "transit-gateway"
    GeoTheme  = "train-station"
    Lab       = "3"
  })
}
